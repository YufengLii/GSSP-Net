...
class JuncUpConvBlock(nn.Module):
    def __init__(self, in_features, up_scale):
        super(JuncUpConvBlock, self).__init__()
        self.up_factor = 2
    ...
    def compute_out_features(self, idx, up_scale):
        return 3 if idx == up_scale - 1 else self.constant_features
...
class NormalHead(nn.Module):
    def __init__(self, input_channels, output_channels=1):
        ...
        self.head = nn.Sequential(
            ...
            nn.Conv2d(input_channels // 4, output_channels,
...
class DexiNed(nn.Module):
    ...
    def __init__(self, cfg):
        ...
        self.JuncPred_output_channels_list= cfg.JuncPred_channels_list
        self.JuncPred_input_channels = cfg.JuncPred_input_channels
        ...
        self.MultiTaskHead = MultiTaskHead(
            input_channels=self.JuncPred_input_channels , output_channels_list=self.JuncPred_output_channels_list)
    ...
    def forward(self, x):
        ...
        # upsampling blocks
        out_1 = self.up_block_1(block_1) # 8 64 256 256
        out_2 = self.up_block_2(block_2) # 8 128 256 256
        out_3 = self.up_block_3(block_3) # 256 128 128
        out_4 = self.up_block_4(block_4) # 512 64 64
        out_5 = self.up_block_5(block_5) # 512 32 32
        out_6 = self.up_block_6(block_6) # 256 32 32
        Edgeresults = [out_1, out_2, out_3, out_4, out_5, out_6]

        # JuncMaps_1 = self.Junchead_1(block_3) # 8 3 128 128
        
        block_cat = torch.cat(Edgeresults, dim=1)  # Bx6xHxW
        block_cat = self.block_cat(block_cat)  # Bx1xHxW

        # return results
        Edgeresults.append(block_cat)

        JuncMaps = self.MultiTaskHead(block_3) # 8 3 128 128

        # lmap = torch.sigmoid(JuncMaps[:, 0:1])
        jmap = torch.sigmoid(JuncMaps[:, 0:1])
        joff = JuncMaps[:, 1:]
        
        ResultJuncMaps = {'jmap': jmap, 'joff': joff}     
        
        
        return Edgeresults, ResultJuncMaps, [block_3, block_4, block_5, block_6]
