...
def stroke2diffvg(strokes, canvas_size=128):
    ...
    for stroke_idx, stroke in enumerate(strokes):
        ...
        if shapes:
            ...
            out.append(render(canvas_size, canvas_size,
                       shapes, shape_groups, samples=2))
        else:
            out.append(th.zeros(canvas_size, canvas_size, 4,
                                device=strokes.device))
...
def graph_link_render_log(all_points, all_alphas, force_cpu=False, canvas_size=512, colors=None, withDraw=True, step=0):

    dev = all_points[0].device
    if force_cpu:
        all_points = [item.to("cpu") for item in all_points]
        all_alphas = [item.to("cpu") for item in all_alphas]    
    eps = 1e-4
    all_points = [ item + eps*th.randn_like(item)  for item in all_points ]

    bs = len(all_points)  
    
    n_out = 3 if colors is not None else 1
    output = th.zeros(bs, n_out, canvas_size, canvas_size,
                      device=all_points[0].device)          
    
    scenes = []
    for k in range(bs):
        shapes = []
        shape_groups = []
        for p in range(all_points[k].shape[0]):
            points = all_points[k][p].reshape(-1,2).contiguous()
            num_ctrl_pts = th.zeros(1, dtype=th.int32)
            width = th.tensor(1.0).to(dev)
            alpha = all_alphas[k][p]
...
def graph_link_render_batch(all_points, all_alphas, force_cpu=False, canvas_size=512, colors=None, withDraw=True, step=0):
...
def graph_link_render_one(all_points, all_alphas, force_cpu=False, canvas_size=512, colors=None):
    ...
    eps = 1e-4
    # all_points = all_points + eps

    n_out = 3 if colors is not None else 1
    output = th.zeros( n_out, canvas_size, canvas_size,
                      device=all_points[0].device)    
    scenes = []
    shapes = []
    shape_groups = []
    for p in range(all_points.shape[0]):
        points = all_points[p].reshape(-1,2).contiguous()
        num_ctrl_pts = th.zeros(1, dtype=th.int32)
        width = th.tensor(1.0).to(dev)
        alpha = all_alphas[p]
        color = th.ones(3, device=alpha.device)
    ...
    return output, scenes
def graph_link_render_one_with_threshhold(all_points, all_alphas, force_cpu=False, canvas_size=512, alpha_threshold=0.01, colors=None):
...
def graph_link_render(all_points, all_alphas, force_cpu=False, canvas_size=512, colors=None, withDraw=True, step=0):
    ...
    n_out = 3 if colors is not None else 1
    output = th.zeros(1, n_out, canvas_size, canvas_size,
    ...
    for p in range(all_points.shape[0]):
        points = all_points[p].reshape(-1,2).contiguous().cpu()
        num_ctrl_pts = th.zeros(1, dtype=th.int32)
        width = th.tensor(1.0).to(dev)
        alpha = all_alphas[p]
        # alpha = th.tensor(1)
        ...
        raster = render(canvas_size, canvas_size, shapes, shape_groups,
                        samples=2)
        if withDraw:
            pydiffvg.imwrite(raster.cpu(), './figure/rendered/' + str(step) + '.png' ,  gamma=2.2)
...
def line_render(all_points, all_alphas, link_thresh=0.5, force_cpu=True,
                canvas_size=512, colors=None):
    ...
    eps = 1e-4
    all_points = [ item*4 + eps*th.randn_like(item)  for item in all_points ]
    ...
    for k in range(bs):
        ...
        for p in range(all_points[k].shape[0]):
            ...
            points = all_points[k][p].reshape(-1,2).contiguous().cpu()
            num_ctrl_pts = th.zeros(1, dtype=th.int32)
            width = th.tensor(1.0).cpu()
            alpha = all_alphas[k][p].cpu()
            if alpha > link_thresh:
                alpha = th.tensor(1)
            else:
                alpha = th.tensor(0)
            color = th.ones(3, device=alpha.device)
...
#     all_points = 0.5*(all_points + 1.0) * 4
